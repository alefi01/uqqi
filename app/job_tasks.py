...
async def build_site(company_id: int):
    ...
    try:
        ...
        # Pro-триал 7 дней — ТОЛЬКО на первый сайт аккаунта (2026-09, решение
        # владельца; недолго побыл «на каждый сайт»). Отметка — users.trial_used.
        # Начисляем ПО ГОТОВНОСТИ, а не при создании: так триал не сгорает на
        # неудачной сборке. Claim-сайты (is_claim, user_id ещё NULL при сборке)
        # получают триал при привязке — там та же проверка trial_used.
        granted = False
