...
# ── Город из адреса ──────────────────────────────────────────────────────────
# Яндекс отдаёт адрес как «Москва, ул. Ленина, 5, этаж 1» — иногда с городом
# впереди, иногда без него вовсе. Раньше в ЛК брали ПОСЛЕДНИЙ компонент, и в
# карточке сайта появлялось «этаж 1». Здесь один разбор на всех.

# Уточнения помещения: это никогда не город.
_ADDR_DETAIL = re.compile(
    r"^(этаж|эт\.|офис|оф\.|помещ|пом\.|кв\.|квартира|корп|стр|строение|литера|"
    r"подъезд|вход|павильон|секция|блок|бутик|модуль|место|цоколь|подвал)",
    re.IGNORECASE)

# Улицы и подобное: тоже не город.
_ADDR_STREET = re.compile(
    r"(^|\s)(ул\.|улица|просп|пр-т|пр-кт|пр\.|проспект|пер\.|переулок|шоссе|ш\.|наб\.|"
    r"набережная|б-р|бульвар|пл\.|площадь|проезд|тракт|тупик|аллея|линия|квартал|"
    r"мкр|микрорайон)(\s|$|\.)",
    re.IGNORECASE)

# Явный маркер населённого пункта — если он есть, берём эту часть.
_ADDR_CITY = re.compile(
    r"^(г\.|город|пгт|п\.|посёлок|поселок|с\.|село|дер\.|деревня|ст-ца|станица|"
    r"хутор|х\.|рп|аул)\s*",
    re.IGNORECASE)
...
def city_from_address(address: str) -> str:
    """
    Город из адреса. Пустая строка, если распознать нечего.

    Порядок: сначала ищем часть с явным маркером («г. Казань», «посёлок …»),
    иначе берём первую «словесную» часть, отбросив улицы, дома и уточнения
    помещения. Страну в начале пропускаем.
    """
    if not address:
        return ""
    ...
    good = []
    for p in parts:
        if _ADDR_DETAIL.match(p):          # «этаж 1», «офис 12»
            continue
        if _ADDR_STREET.search(p):         # «ул. Ленина»
            continue
        if not re.search(r"[A-Za-zА-Яа-яЁё]", p):   # «5», «12А»
            continue
        good.append(p)

    for p in good:
        if _ADDR_CITY.match(p):
            return _ADDR_CITY.sub("", p).strip() or p
