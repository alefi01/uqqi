...
# uqqi-адаптация: правило про тире ОТКЛЮЧЕНО по умолчанию. В русской типографике
# тире обязательно (прямая речь, пауза, приложение) и не является признаком
# машинного текста. Включить можно флагом --dash (для англоязычной копии).
def main(argv):
    argv = list(argv)
    dash_rule = "--dash" in argv
    argv = [a for a in argv if a != "--dash"]
